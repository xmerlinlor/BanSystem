...
ctx.telegram ? ctx.botInfo?.id : null,
ctx.from.id
).catch(() => null);
...
📸 Pᴀʏ Fɪʀsᴛ, Tʜᴇɴ Sᴇɴᴅ Yᴏᴜʀ Pʀᴏᴏғ Wɪᴛʜ Yᴏᴜʀ Rᴇǫᴜᴇsᴛ.
...
await ctx.answerCbQuery();
  ...
  if (!isOwner(ctx.from.id)) {
    return ctx.reply("❌ Oɴʟʏ Aᴜᴛʜᴏʀɪᴢᴇᴅ Oᴡɴᴇʀs Cᴀɴ Aᴄᴄᴇss Tʜɪs.");
  }
...
await ctx.answerCbQuery();
  ...
  if (!payments.length) {
    return ctx.reply("💳 Nᴏ Pᴀʏᴍᴇɴᴛ Rᴇᴄᴏʀᴅs Fᴏᴜɴᴅ.");
  }
...
await ctx.answerCbQuery();
  ...
  const requests = Object.keys(data.requests).length;
  const payments = Object.keys(data.payments).length;
  const referrals = Object.keys(data.referrals).length;
  const bans = Object.keys(data.bans).length;
...
await ctx.answerCbQuery();
  ...
  if (String(request.userId) !== String(ctx.from.id)) {
    return ctx.reply("❌ Tʜɪs Iѕ Nᴏᴛ Yᴏᴜʀ Rᴇǫᴜᴇsᴛ.");
  }
...
⚠️ Tʜᴇ Aᴄᴛᴜᴀʟ Pʟᴀᴛғᴏʀᴍ Aᴄᴛɪᴏɴ Iѕ Nᴏᴛ Yᴇᴛ Cᴏɴɴᴇᴄᴛᴇᴅ Tᴏ Tʜɪs Rᴇᴠɪᴇᴡ Sʏsᴛᴇᴍ.`
);
...
// Ignore already answered callbacks.
}
});
